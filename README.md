...
- **Sessions**: every transcript, with its size, token use, and how full its context window got
- **Projects**: sessions rolled up per working directory, flagging directories that no longer exist
- **Traffic**: tokens per day, split by cache behaviour and by model
- **Prune**: reclaimable data, such as superseded transcripts and per-session data left behind
...
```sh
uv run claudetop          # from a checkout
uv tool install .         # put `claudetop` on your PATH
```
...
Actions: `s` sort (or click a column header) · `d` delete · `m` rehome · `x` prune ·
`O` include sessions whose project dir is gone · `M` include their auto-memory ·
`r` rescan · `q` quit
